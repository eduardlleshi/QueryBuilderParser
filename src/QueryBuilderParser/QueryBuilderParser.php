...
class QueryBuilderParser
{
	 ...
	 */
	public function __construct( array $fields = [], array $raw_fields = [], array $ignored_fields = [] )
	{
		$this->fields         = $fields;
	...
	protected function makeQuery( $query, stdClass $rule, $queryCondition = 'AND' )
	{
		...
		/*
		 * Remove ignored fields from the query
		 */
		if ( in_array( $rule->id, $this->ignored_fields ) ) {
			return $query;
		}
